...
[data-testid="stSidebarNav"], a[href*="streamlit.io"], a[href*="github.com"] {display:none !important;}

:root{
 --bg:#07101f; --surf:#0d1a2e; --card:#112038; --bdr:rgba(255,255,255,.07);
 --acc:#f97316; --acc2:#c2410c; --blue:#3b82f6; --txt:#f0f4f8; --muted:#5a7090;
}
...
.stButton>button {
 background:linear-gradient(135deg,var(--acc),var(--acc2))!important; color:#fff!important; border:none!important;
 font-weight:800!important; border-radius:12px!important; width:100%!important; padding:.65rem .8rem!important;
}
...
.stTextInput input,.stSelectbox>div>div,.stMultiSelect>div>div,.stNumberInput input{
 background:var(--card)!important; color:var(--txt)!important; border:1px solid var(--bdr)!important; border-radius:10px!important;
}
...
.scoreboard{
 background:linear-gradient(135deg,var(--surf),var(--card)); border:1px solid rgba(249,115,22,.15); border-radius:18px;
...
.score-num{font-family:'Bebas Neue',sans-serif; font-size:88px; line-height:1; color:#fff;}
...
.badge{display:inline-block; padding:3px 8px; border-radius:999px; font-size:10px; font-weight:800;}
...
.smallnav .stButton>button{padding:.45rem .55rem!important; font-size:12px!important; border-radius:10px!important;}
.bigpoint .stButton>button{padding:1rem .8rem!important; font-size:20px!important; border-radius:14px!important;}
...
@media(max-width:768px){
  .block-container{padding:.35rem .4rem 5.2rem!important;}
  .score-num{font-size:68px!important;}
  .tname{font-size:14px!important;}
  .bigpoint .stButton>button{font-size:17px!important; padding:.9rem .7rem!important;}
  .smallnav .stButton>button{font-size:11px!important; padding:.4rem .45rem!important;}
}
</style>
...
def do_point(m: Match, winner: str, data: dict) -> Match:
    ...
    if m.target == 39 and (m.scA == 39 or m.scB == 39):
        ...
        if m.sA == 2 or m.sB == 2:
            ...
            m.setno += 1
            m.scA = 0; m.scB = 0; m.subA = 3; m.subB = 3; m.toA = 1; m.toB = 1
    ...
    if m.over and not m.ended:
        ...
        data["history"].append({
            "id": m.id, "date": m.started, "tA": m.tA, "tB": m.tB, "sA": m.sA, "sB": m.sB,
            "winner": m.tA if m.winner == "A" else m.tB, "tnm": m.tnm, "trd": m.trd,
...
if st.session_state.role == "viewer":
    ...
    st.markdown("## Viewer")
    if matches:
        options = {f"{m.tA} vs {m.tB} · {m.tnm or 'Live Match'} · {m.id}": m.id for m in matches}
        ...
        if current:
            for i, lbl in enumerate(labels):
                if options[lbl] == current:
                    idx = i
                    break
        selected_label = st.selectbox("Choose Match", labels, index=idx)
        ...
        with b3:
            if st.button("📜 History", key="v_hist", use_container_width=True):
            ...
            if hist:
                for h in reversed(hist):
                    with st.container(border=True):
                        ...
                        if h.get("set_scores"):
                            st.write(f"Set-wise score: {h['set_scores']}")
                        if h.get("player_points_A"):
                            st.write("Player Points A:", h["player_points_A"])
                        if h.get("player_points_B"):
                            st.write("Player Points B:", h["player_points_B"])
            else:
...
# ---------- ADMIN NAV ----------
nav_items = [("score","🏸 Score"),("stats","📊 Stats"),("history","📜 History"),("tournament","🏆 Tournament"),("users","👥 Users"),("admin","⚙️ Admin")]
...
if tab == "score":
    if selected_match is None:
        ...
        c0, c00 = st.columns(2)
        with c0: tnm = st.text_input("Tournament Name", key="tnm")
        with c00: trd = st.text_input("Round", key="trd")
        c1, c2 = st.columns(2)
        with c1: tA = st.text_input("Team A", key="tA")
        with c2: tB = st.text_input("Team B", key="tB")
        st.markdown("### Players")
        ca, cb = st.columns(2)
        allA, allB = [], []
        with ca:
        ...
        s1, s2 = st.columns(2)
        with s1: mpA = st.multiselect("Team A Starters", allA_f, default=allA_f[:PLAYERS], max_selections=PLAYERS)
        with s2: mpB = st.multiselect("Team B Starters", allB_f, default=allB_f[:PLAYERS], max_selections=PLAYERS)
        errs = []
        oA2, oB2 = [], []
        ...
        if st.button("▶️ Start Match", use_container_width=True, disabled=bool(errs)):
            ...
            m = new_match(mid, st.session_state.username, _safe(tA, "Team A"), _safe(tB, "Team B"),
        ...
        with mt1: st.metric("Serving", m.tA if m.srv=="A" else m.tB)
        with mt2: st.metric("Server", current_server(m))
        with mt3: st.metric("Target", m.target)
        with mt4: st.metric("Created By", m.created_by)
        ...
        with r1[3]:
            if st.button("🔁 Subs", use_container_width=True):
                ...
                st.rerun()
        with r1[4]:
            if st.button("🗑️ End/Delete", use_container_width=True):
            ...
            with st.container(border=True):
                ...
                with b1:
                    if st.button(f"Set {m.tA}", use_container_width=True):
                        m.scA = int(new_a); maybe_update_target(m); save_match(m, data); st.rerun()
                with b2:
                    if st.button(f"Set {m.tB}", use_container_width=True):
                        m.scB = int(new_b); maybe_update_target(m); save_match(m, data); st.rerun()
        ...
        if st.session_state.show_subs_panel:
            with st.container(border=True):
                ...
                for team, col in [("A", c1), ("B", c2)]:
                    ...
                    with col:
                        st.markdown(f"**{tname}** <span class='badge b-green'>Subs {subs}</span> <span class='badge b-blue'>T/O {tos}</span>", unsafe_allow_html=True)
                        ...
                        if subs > 0 and bench:
                            ...
                            if st.button(f"Sub {tname}", key=f"sub_{team}", use_container_width=True):
                                do_sub(m, team, on_p, off_p, data); st.rerun()
                        if tos > 0:
                            if st.button(f"Timeout {tname}", key=f"to_{team}", use_container_width=True):
                                do_timeout(m, team, data); st.rerun()
...
# ---------- TOURNAMENT ----------
    ...
    st.info("Tournament bracket kept separate from live multi-match control. Use this for simple bracket notes if needed.")
...
# ---------- USERS ----------
    ...
    with st.container(border=True):
        ...
        is_admin = st.checkbox("Give Admin Access", key="add_is_admin")
